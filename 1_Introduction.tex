...
% E: I was a bit worried at this point, because your paper is not about spatial distributions.  But then I saw that you are setting up a nice contrast with the next paragraph.  Perhaps the above could be substantially shortened so that the paragraphs can be merged and the logic is more immediately apparent to the reader.
...
The question thus remains, given everything else is equal, how does thermal performance curve change with body size? % E: Nice.
...
% E: I noticed at this point that the empirical niche envelope models are not mentioned.  I agree that I now don't see where they would fit.  Just wondering if that's okay.

In this work we build a theoretical model to investigate how thermal performance varies with body size for cold blooded animals.  % E: Perhaps a bit too broad here, given that you soon after narrow the scope to some insects.
...
The philosophy of the model is the same as energy budget model, here we define performance as difference between total energetic gain and energetic cost. % E: "net energy" perspective is the same, but "philosophy" about abstraction/detail is opposite
