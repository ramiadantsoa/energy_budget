...
There is no previous theoretical work that shows clearly how performance (e.g., optimal temperature, critical minimum or maximum temperature) changes with body size.
...
Unlike classical energy budget models \citep[e.g.,][]{Kooijman2009}, we opt for a process rather than pattern oriented modeling.
Furthermore, we put a strong emphasis on the role of thermoregulation for heterotherms in shaping performance by analyzing in detail the temporal process of warm-up.
...
We do not mean that these are the only processes that shape thermal performance---other factors like competition, heat stress, and cold tolerance can be just as important---but the net energy gain can be a first proxy for the actual (realized) thermal performance.
...
Whereas the exponent of the resting metabolic rate is interesting on its own, our model suggests that, when comparing performance across body size, the exponent of the foraging rate $b_3$ is even more important.
We assumed that the exponent is always positive, which means that foraging rate always increases with body size, but the increase can be concave or convex (\cref{fig1}b).
...
The converse is also true that convexity benefits larger individuals (\cref{fig2,fig5}c).
Unlike the exponent of resting metabolic rate, there is no clear value for the exponent for foraging rate (see section Model Description: Power law and parameter justifications ). % E:  \label and \cref also work for sections, by the way, T: I did not work after few tries, maybe because I don't number the section.
...
If resource quality is too low, there is not enough energetic gain and individuals of all size have negative net energy gain.
In contrast, if resource quality is high, individuals of all size have a positive net energy gain but the low mass specific metabolic cost confers the advantage to large individuals.
...
Although we focus on individual performance, the results are potentially at larger scale.
...
A last example where the process warm-up can play a significant role is in regulating diel activity.
For instance, data has shown that a community of dung beetles are accurate in timing their activities \citep{Halffter1966, Caveney1995}.
Although the generally proposed mechanism of temporal partitioning is to reduce competition,  we found a significant difference in the duration of warm-up depending on the hour of the day especially soon after sunrise and depending on body size.
 In such a competitive community where resources can be depleted fast \citep{Hanski1991} and optimizing warm-up time to arrive at the source can become crucial.
