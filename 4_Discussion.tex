...
Concavity favors smaller individual, convexity benefits large and differences are accentuated with increasing temperature (\cref{fig:2}).
...
However, for ectotherm wind can create enough convection that being small is no more optimal.
An important parameter is conductance which control heat exchange between the thorax and the surface.
There are not much data about conductance although  \citet{Bartholomew1978} found  cooling rates are similar although conductance is controlled by very different processes for dung beetles and moth and bees. %the air beneath the elythra for dung beetles and by an insulating pile for moths and bees,.
Nevertheless, the homogeneity of such value would be surprising. % as coloration can make a difference in heat exchange \citep{Forsman2002}. 
For instance, we found that depending on the timing of warm-up, the optimal conductance should be higher for endotherm  when warm-up occurs in the early morning  to increase the heat absorption but the optimal conductance should be lower afterwards to increase heat retention.
Such low conductance in part can be a problem because insects also need to dissipate heat during activities.
...
We relate our model with empirical studies.
First, studies on endothermic dung beetles have shown than below a certain mass (about 2 g), individual becomes thermoconformer i.e. not capable of endogeneous thermoregulation \citep{Bartholomew1978, Verdu2006}.
We actually found  a threshold condition.
In \cref{fig:4}c (dashed line with low convection) warm-up can be completed at any given temperature (here above freezing temperature) above a certain body mass.
...
This can occur if thermoregulation capacity and thermal niche matches.
...
Our model reveals that maybe lower warm-up ability for endothermic insect does not allow the small ones to forage during colder times.
Third, data has shown that a community of dung beetles can be exact on when they start to be active \citep[e.g.,]{Halffter 1966,Caveney1995}.
...
In this study, we compared performance across body sizes by looking at their absolute net energy gain.
Clearly, net energy gain is a very simplistic approximation of performance and does not mean fitness
\citet{Kozlowski1996} has already pointed out that energetic definition of fitness is wrong such that adding size dependent mortality can shift optimum body size.
Conversion to fecundity, assimilation and more all can affect the thermal performance.
...
Our main goal is to have a realistic, understandable and measurable parameter so yet not too specific that would still allow to get general insight. 
...
Although the model is focused on looking at individual, the model helps to pinpoint possible mechanisms behind  some macroecological patterns.
For instance, it can be used test various reasons for seeing or not Bergmann's rule \citep{Blackburn1999}.
Whether the constraints is on resources availability, when resources are scares, the larger absolute cost can lead to a decrease in body size.
Thermoregulation can be important as it determines whether the individual can be active or not and indeed Cushman 1993 voiced that slow ability to warm-up can disadvantage large.
There is a long list of mechanisms for Bergmann's rule an approach like our can allow testing.
...
In summary, our integrative model underscore that parameters such as foraging rate, conductance that would require empirical attentions.
Theoretically, there is no single relationship on how thermal performance changes with body size.
It can increase, decrease or shifts depending on the ecological settings especially: metabolic rate, foraging rate, thermoregulation, and resource availability
